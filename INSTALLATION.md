...
# Скачиваем модель (~4.7 GB, займёт несколько минут)
ollama pull llama3.1:8b
```

Проверка:
```bash
ollama run llama3.1:8b "Привет, скажи OK"
```
...
## Шаг 7 — Настройка Claude API (опционально)

Для глубокого архитектурного анализа через Claude — нужен API ключ Anthropic.
...
4. Открой **⚙ Настройки** в UI → введи ключ → сохрани

Ключ хранится в `.env` файле локально и никогда не попадает в git (`.gitignore`).
