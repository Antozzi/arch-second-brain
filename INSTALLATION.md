...
> **Русскоязычные документы:** модель `qwen2.5` точнее работает с кириллицей. Её
> не обязательно ставить заранее — при загрузке проекта система определяет долю
> кириллицы и предлагает установить `qwen2.5:7b` в один клик (а `start.sh` /
> `start.bat` спросят про это в deployment-визарде).
