...
| `[TIMEOUT]` | Модель не ответила за отведённый `TIMEOUT` (зависит от ОЗУ: 180–600с) |
...
### Русскоязычные документы — модель путает или теряет смысл

Система определяет долю кириллицы в `/raw` и при загрузке проекта предлагает
установить `qwen2.5` (точнее с русским). Если предложение пропустил:
